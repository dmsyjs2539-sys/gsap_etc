...
//처음 화면에 들어오기 전에 기본 stikcy 이미지를 1번으로 세팅
changeBg(`asset/info1.png`)

const pinBg = document.getElementById('pin_bg');
const photos = gasp.utils.toArray('.photo');
...
const pinTl = gsap.timeline({
 scrollTrigger : {
  trigger  : '.pin_scene',
  start:'top top',
  //1800px 스크롤 하는 동안에 타임라인이 진행되는 것
  end : '+=1800',
  //섹션에 화면에 고정해서 사진이 올라오는 무대를 만든다
  pin : true,
  //스크롤 위치와 애니메ㅣ션 진행도를 연결
  scrub:true,
...
//사진마다 살짝 다른 각도와 giltch 효과를 줘서 디자인 갤러리 처럼 보이게 한다
photos.forEach((photo,index)=>{
  //사진이 등장하기 직전에 레이어 순서와 giltch 클래스를 조정
  pinTl.add(()=>{
   //뒤에 나온 사진이 앞쪽에 쌓이도록 z인덱스 올림
   photo.style.zIndex = String(100+index);
   photo.classList.add('glitch');
   //0.4초 뒤 glitch 클래스를 제거해서 효과를 끝냄
   gasp.delayedCall(0.4,()=>photo.classList.remove('glitch'));
  },index *0.22);
...
//가로 스크롤 색션에서 실제로 옆으로 움직여야 되는 거리를계산
//트랙의 전체 너비에서 화면에 이미 보이는 너비를 빼면 이동거리만 남음
function totalWidth(){
  const warp = document.querySelector('.horizontal_section');
...
const horizontalTween = gsap.to('.track',{
  //track을 왼쪽으로 이동시켜 오른쪽 카드들이 보이게 한다
  x: ()=> -totalWidth(),
  ease:'none',
  scrollTrigger:{
    trigger:'.horizontal_section',
    start:'top top',
    //실제 이동거리보다 더 길게 잡아 앞뒤 여백 스크롤을 만든다
...
//패널 안에 이미지들을 하나씩 다른 preset 으로 등장
...
//이미지나 폰트가 늦게 로드되면 섹션 위치가 달라질 수 있음
//그때 스크롤 트리거 위치 계산을 다시 맞춰주는 함수
...
window.addEventListener('load', ()=>{
  ...
  if(document.fonts){
    //폰트가 모두 준비 된 뒤 스크롤트리거 위치를 다시 맞춘다
