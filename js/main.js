...
//z카드 안쪽 레이러를 마우슨 위치에 맞춰 사락기울인다
//rotatex rotateY값만 바뀌
document.querySelectorAll('.tit_card').forEach((card)=>{
  const layer = card.querySelector('.tit_layer')
  //레이어가 없으면 이 카드는 건너 뛴다
  if(!layer) return;

  //카드 위에서 마우스를 움직일 떄 기울기 값을 계산
  card.addEventListener('mousemove', (e)=>{
    const rect = card.getBoundingClientRect();
    //카드 안에서 마우스가 가로로 몇 퍼센트 위치인지 계산
    const px = (e.clientX - rect.left) / rect.width;
    const py = (e.clientY - rect.top) / rect.height;
    //위 아래 위치를 rotateX 각도로 바꾼다
    const rx = (py - 0.5 ) * -15;
    const ry = (px - 0.5) * 22;

    //계산한 각도를 안쪽 레이어 transform에 적용시킴
...
//sticky 박스의 이미지를 바꾸면 딱딱해서 살짝 줄였다가 다시 키우며 교체
